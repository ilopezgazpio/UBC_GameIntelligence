...
class DuelingNeuralNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.advantage_network = nn.Linear(hidden_layers_size[-1], output_layer_size)
        self.value_network = nn.Linear(hidden_layers_size[-1], 1)
        print(self.base_network)
        print(self.advantage_network)
    ...
    def forward(self, x):
        ...
        value = self.value_network(base_results)
        advantages = self.advantage_network(base_results)

        # Normalize
        advantages_mean = advantages.mean(dim=-1, keepdim=True)

        return value + (advantages - advantages_mean)
