class PERDetDQN_RL_Agent(DeterministicDQN_RL_Agent):
    ...
    # Cada vez que el agente hace una transición se llama a esta función
    def __PER_DQN_bellman_update__(self, old_state: State, action, new_observation : gym.Space, reward: float, terminated, truncated):
        ...
        # Saco las experiencias a entrenar
        result = self.memory.sample()
        if result is None:
            return

        # Transiciones para el entrenamiento
        ...
        # Calcular TD errors y target values
        with torch.no_grad():
            q_values_next = self.QNetwork(new_observation_batch)
            max_q_next = torch.max(q_values_next, 1)[0]
            target = reward_batch + (1 - (terminated_batch | truncated_batch)) * self.gamma * max_q_next
